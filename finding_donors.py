...
# *** -What are the strengths of the model; when does it perform well?***
# 
# It is robust in noisy training data and effective if the training data is large. No training phase is required, and it learns complex models easily
# 
# *** -What are the weaknesses of the model; when does it perform poorly? ***
# 
...
# *** - What makes this model a good candidate for the problem, given what you know about the data?***
# 
# Since we already have the income data we could easily create k-nearest neighbors around the 50k+ people
...
# 
# This single tree that we make based on the classifications may work really well, but the problem is decision trees tend to overfit the data and not perform well on new data. Random forest uses a bunch of random decision trees with lots of mixed variables; these are the weak learners. Then it polls all these random decision trees to get the most common answers, this is the strong learner. It then takes the polled data and creates classifications. Because the model uses lots of random decision trees its a more balanced and flexible model. Random forests thus are known to reduce overfitting and perform better than regular decision trees.
...
# **Answer:**
# 
# I believe what would be most important in determining if someone makes at least 50k would probably be capital gain, occupation, education-num, age, and hours-per-week in that order. The reason I think this is because capital gains are a definite indicator of income, occupations can be standardized and salaries can be fairly estimated. Also a persons education and age wil play a very important factor as most people receive pay raises as they age. Hours per week are important because it shows us if they are employed full-time.
...
# **Answer:**
# 
# These features are actually very similar! The highest weight does go to capital gains, and I had 4/5 of the features correct. Capital loss is also an important feature that I did not include and more important than the occupation type, which I chose.
# 
